...
**Not fully completed.** Manifest V3 foundations, functional CI stabilization, Archiver 8 / Redux 5 modernization and the automated migration/runtime compatibility pass are complete. The CAD cross-browser cookie retyping and the maintained `@types/firefox-webext-browser@143.0.0` dependency migration are complete and verified. The next TypeScript/toolchain step is to resolve the React-Redux/Redux TypeScript-7 declaration mismatch, then continue with Jest/type-declaration modernization, the ambient const-enum/Jest-transform dependency and remaining stricter application errors. Real historical-profile upgrade validation and packaged-browser release-candidate checks remain open.