...
The pinned release-candidate source commit is now `b5cfd87f`, generated after synchronizing the hardened current `3.X.X-Branch` base `3e061b7f`. Regular functional CI is green in all three event paths: push `32440492934`, `pull_request_target` `32440493862`, and pull request `32440495062`. Pull-request CI produced replacement RC artifact `9432252522` (GitHub wrapper digest `sha256:ff8c7495404f6b0d950ecfe91e5de93f6cb86770bd96d07cd252c6b521102ddd`) containing the same-source Chromium ZIP, Firefox XPI and `SHA256SUMS.txt`. That exact source passed TypeScript 7 typecheck, 37/37 Jest suites (577/577 tests), lint, Firefox archive validation, Chromium archive validation, and Firefox ZIP/XPI byte identity through the permanent CI gates. The previous `53209f36` / artifact `9431860563` candidate is superseded because the base changed afterwards. The branch may advance through documentation-only commits without invalidating this pinned candidate; any further change to the base or candidate-affecting runtime/source, manifest, build/packaging, dependency or toolchain state requires a replacement RC and repetition of the affected gates. PR #1 currently reports `mergeable_state: clean`; regular CI, CodeQL and JavaScript/TypeScript/Actions analysis are green. The separate `github-advanced-security` AI check still fails before repository analysis because the GitHub-hosted agent requests an unsupported model; this remains classified as an external GitHub service/configuration problem rather than a functional repository defect.
...
- There is currently no base-drift blocker: PR #1 targets current `3.X.X-Branch` base `3e061b7f`, and pinned replacement RC source `b5cfd87f` was generated after that base was synchronized into the modernization branch. GitHub currently reports `mergeable_state: clean`; all regular CI/code-analysis checks are green except the known external `github-advanced-security` AI agent, which fails before repository analysis. Documentation-only commits after the candidate source do not invalidate artifact `9432252522`; any further base change or candidate-affecting runtime/source, manifest, build/packaging, dependency or toolchain change requires a replacement candidate and repetition of the affected gates.
