...
- TypeScript 7 is no longer blocked by npm peer resolution, the removed legacy module-resolution setting, or WebExtension typing maintenance. The CAD-local `CadCookie` shape and the `web-ext-types@3.2.1` → `@types/firefox-webext-browser@143.0.0` migration are implemented and verified. `skipLibCheck` deliberately isolates third-party declaration incompatibilities at this intermediate stage without suppressing application use-site checking. The React-Redux/Redux generic-constraint mismatch is now isolated at the third-party declaration boundary and the TS7 whole-program compile is clean. Jest/type declarations and the ambient runtime-enum dependency are now modernized; the remaining TypeScript 7 work is the coordinated compiler/transform configuration migration and any compatibility defects exposed by the committed TS7 toolchain.
...
**Not fully completed.** Manifest V3 foundations, functional CI stabilization, Archiver 8 / Redux 5 modernization and the automated migration/runtime compatibility pass are complete. The CAD cross-browser cookie retyping and the maintained `@types/firefox-webext-browser@143.0.0` dependency migration are complete and verified. The React-Redux/Redux TypeScript-7 declaration boundary is now verified clean without a React runtime-major change. The next TypeScript/toolchain step is removing obsolete `ts-jest`-specific configuration and committing the coordinated TypeScript 7 compiler/transform dependency changes, followed by the full validation matrix on that committed toolchain. Real historical-profile upgrade validation and packaged-browser release-candidate checks remain open.