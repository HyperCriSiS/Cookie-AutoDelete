...
- [ ] Resolve current base drift / merge conflicts between `modernization-p0` and `3.X.X-Branch` before treating any candidate as the final manual-test RC. GitHub reports PR #1 as dirty and an `update_pull_request_branch` attempt from head `2ad69cf8` returned HTTP 422 (`merge conflict between base and head`). Base-only dependency/action upgrades were audited and are already semantically present in the modernization branch; the remaining integration step must preserve the modernization lockfile/workflow state rather than blindly replaying stale base diffs.
- [ ] Regenerate the final same-source `release-candidate-packages` artifact after the base-sync conflict is resolved. The previous combined RC artifact `9390334806` predates later historical-schema/CI-hardening commits. Green head `2ad69cf8` does have fresh Firefox artifact `9391597687` and Chromium artifact `9391598130` from PR-target run `32326613299`, but these are not the final combined RC while PR #1 remains non-mergeable.
...
- PR #1 currently has base drift that GitHub cannot auto-sync (`update_pull_request_branch` returns HTTP 422 merge conflict). Final manual RC validation must wait for a mergeable/base-synchronized modernization head so the tested source does not change during integration.
