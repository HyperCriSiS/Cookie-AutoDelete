...
- [ ] Evaluate and migrate to TypeScript 7 only through the modernization branch with full install/test/build validation; do not merge the open major-version Dependabot PR directly.
  ...
  - [ ] Modernize the WebExtension and React/Redux type packages/configuration to versions compatible with the existing runtime libraries and TypeScript 7, then re-run the whole-program compile to separate declaration-package failures from real application typing defects.
...
- TypeScript 7 is no longer blocked merely by npm peer resolution once `ts-jest` is removed. The removed legacy `moduleResolution=node10` setting can be moved to `Node16`, but the next compile gate is the outdated declaration ecosystem: current WebExtension and React/Redux typings generate broad TS7 errors. A compiler-independent SWC transform also remains non-equivalent because the codebase relies on ambient `declare const enum` inlining (`browserName`, `SiteDataType`, `SettingID`, `ListType`, `ReasonClean`, `OpenTabStatus`).
...
**Not fully completed.** Manifest V3 foundations, functional CI stabilization, Archiver 8 / Redux 5 modernization and the automated migration/runtime compatibility pass are complete. TypeScript 7 is now proven to progress past the old `moduleResolution=node10` gate with `Node16`; the next concrete migration step is modernizing the stale WebExtension and React/Redux type declarations, then resolving the ambient const-enum/Jest-transform dependency and remaining stricter TS7 application errors. Real historical-profile upgrade validation and packaged-browser release-candidate checks remain open.
